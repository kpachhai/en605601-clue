...
public class StartGame extends JFrame implements MouseListener, ActionListener, MouseMotionListener{
    ...
    public void actionPerformed(ActionEvent e){
        ...
        //If Human Player's turn and not disproving.
            ...
            if (e.getSource() == hub.getMakeAccusation())
                //Disable StartGame until Accusation Selection is complete.
                setEnabled(false);

            //If human player clicked on Assumption button.
            else if (e.getSource() == hub.getMakeAssumption())
                //Disable StartGame until Assumption Selection is complete.
                setEnabled(false);
    ...
    private Rooms roomClicked(Point point){
        ...
        ArrayList<Rooms> canMoveToRooms = new ArrayList<Rooms>();
        ...
        for (Rooms rooms : Rooms.values())           
            if (rooms.getBoundaryBox().contains(point) && rooms != currentRoom && canMoveToRooms.contains(rooms)) { return rooms;}
        return null;
    }

    /** nextTurn increments the turn count by one, and shows appropriate
  prompts, depending on if next turn is a human or AIPlayer.  */
    ...
    /** displayMovement displays the results from a dice Roll for both human and AIPlayer.  */
    ...
    /** performAIAction enacts AIPlayer's next move based on int provided
  by AIPlayer class logic.
